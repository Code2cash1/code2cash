...
export function ProjectDashboard({
  ...
  const preparedProjects = useMemo(() => {
    ...
    return list;
  }, [dataProjects, query, activeSortBy, activeSortDir, activeStatusFilter]);
  ...
  return (
    <div className={cx(
      "pd-container flex flex-col h-screen bg-[#030303]",
      className
    )}>
      <header className={cx(
            ...
            <a
              ...
              className={cx(
                ...
                l.active
                  ...
                  : "bg-[#0a0a0a] text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-700"
              )}
              ...
              return (
                ...
                >
                  ...
                  {!isEditing ? (
                    ...
                    <form className={cx("mt-3 grid gap-2", viewMode === "list" ? "w-full grid-cols-2" : "grid-cols-1")}>
                      <input className={cx(
                        "rounded-lg ring-1 ring-slate-200 dark:ring-slate-700",
                        "bg-[#0a0a0a]",
                        spacing.button.sm
                      )} value={editDraft?.name ?? ""} onChange={(e) => setEditDraft((d) => ({ ...(d as Project), name: e.target.value }))} placeholder="Project name" required />
                      <input className={cx(
                        "rounded-lg ring-1 ring-slate-200 dark:ring-slate-700",
                        "bg-[#0a0a0a]",
                        spacing.button.sm
                      )} value={editDraft?.subtitle ?? ""} onChange={(e) => setEditDraft((d) => ({ ...(d as Project), subtitle: e.target.value }))} placeholder="Subtitle" />
                      <div className={cx("col-span-full flex items-center", spacing.gap.xs, "mt-2")}>
                        <button type="submit" className={cx("rounded-lg bg-indigo-600 text-white hover:bg-indigo-500", spacing.button.sm)}>
                          Save
                        </button>
                        <button type="button" onClick={cancelEdit} className={cx("rounded-lg ring-1 ring-slate-300 dark:ring-slate-700", spacing.button.sm)}>
                          Cancel
                        </button>
                      </div>
                    </form>
                  ...
                  {!isEditing && (
                    <div className={cx("mt-4 flex items-center justify-between", viewMode === "list" ? "w-full" : "")}>
