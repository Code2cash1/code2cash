...
export default function InternshipApplicationForm() {
    ...
    const [selectedProgramPrice, setSelectedProgramPrice] = useState("₹1,500");
    ...
    useEffect(() => {
        const fetchPrograms = async () => {
            try {
                ...
                if (preSelectedProgram && data.length > 0) {
                    const found = data.find((p: Program) => p.title === preSelectedProgram);
                    if (found) setSelectedProgramPrice(found.price);
                }
            } catch (error) {
    ...
    const handleProgramChange = (value: string) => {
        ...
        const found = programs.find(p => p.title === value);
        if (found) setSelectedProgramPrice(found.price);
    };
                                        ...
                                        <div className="space-y-4">
                                            ...
                                            <Select required value={formData.program} onValueChange={handleProgramChange}>
                                                ...
                                                <SelectContent className="bg-[#0a0a0a] border-white/10 text-white">
                                                    {programs.map(p => (
                                                        <SelectItem key={p._id} value={p.title} className="focus:bg-[#31a39c]/20 focus:text-[#31a39c]">{p.title}</SelectItem>
                                                    ))}
                                                </SelectContent>
                                            </Select>
                                        </div>
                            ...
                            <div className="bg-gradient-to-br from-[#31a39c]/20 to-[#31a39c]/10 rounded-2xl p-6 border-2 border-[#31a39c]/30">
                                ...
                                <div className="space-y-3">
                                    <div className="flex items-baseline gap-2">
                                        <span className="text-4xl font-bold text-white">{selectedProgramPrice}</span>
