...
export default function CertificateManagement() {
    ...
    const updateRequestStatus = async (id: string, status: string) => {
        try {
            await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/reissue/${id}`, {
                method: 'PATCH',
    ...
    const deleteReissueRequest = async (id: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/reissue/${id}`, { method: 'DELETE' });
            if (res.ok) {
                fetchReissueRequests();
            } else {
    ...
    const saveCertificate = async () => {
        setIsSaving(true)
        try {
            const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/certificates`, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
            ...
            if (response.ok) {
                ...
                const certId = result.data.certificateId
                toast.success(`Certificate Saved Successfully! ID: ${certId}`)
                setCertForm({ internName: "", email: "", domain: "", duration: "", college: "", startDate: "", endDate: "" })
    ...
    const deleteCertificate = async (id: string) => {
        ...
        try {
            const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000'}/api/certificates/${id}`, { method: 'DELETE' })
            if (res.ok) {
